...
**Propagation (#27):** upgrades now reach existing users automatically. Bump `KOS_SEEDS_VERSION` (same file) whenever a seed is added; during schema migration every stored preset list is unioned with the shipped defaults — missing entries are appended with their shipped enabled flag, while user-added and user-disabled entries stay untouched. Two caveats, both accepted for v1: a seed the user deleted outright resurfaces on the next upgrade, and edits to an already-shipped seed (value/enabled change rather than addition) do not propagate. The regression harness [`tools/diag/seed-migration.mjs`](../tools/diag/seed-migration.mjs) pins these semantics.
